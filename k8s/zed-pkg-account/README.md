...
- it exposes `app.zpkg.net` to the Maud/HTMX web service and `api.zpkg.net` to the Axum API through the cluster's nginx ingress class;
- it requests one cert-manager TLS secret, `zed-pkg-public-tls`, for both hosts.
...
9. DNS for `app.zpkg.net` and `api.zpkg.net` resolves to the ingress and cert-manager has issued `zed-pkg-public-tls`.
