...
zed-api-server will issue a compact ES256/P-256 resource-capability JWT only after canonical Shared Auth proof-policy admission and Zed package ACL authorization, with:
...
A v1 capability lives for at most five minutes. The capability uses a Zed-owned ES256/P-256 resource-capability signing key/JWKS lifecycle, separate from Shared Auth identity JWTs. The Cloudflare verifier receives
a pinned/cached Zed capability JWKS snapshot from deployment configuration and performs no
...
2. implement the Zed issuer endpoint after Shared Auth proof-policy + Zed ACL admission;
3. implement provider credential brokers;
4. consume Shared Auth reconciliation/revocation proof policy and add Zed capability key-rotation/replay policy;
